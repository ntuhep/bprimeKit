...
( const edm::ParameterSet& iConfig, const LepInfoBranches& lep, 
  const JetInfoBranches& jet, const VertexInfoBranches& vtx ): 
       _leptons( &lep ),
...
int objectSelector::qualityLep( const int lepIdx, const int vtxIdx ) const
{
   ...
   if( MUON == _leptons->LeptonType[lepIdx] ) {
      ...
      if( _debug ) { 
         printf( "\tmuon: type=%i chi2=%2.3f muhits=%i tkhits=%i d0=%2.3f dz=%2.3f pixlayers=%i matches=%i code=%i\n", 
                 _leptons->MuType[lepIdx], _leptons->MuGlobalNormalizedChi2[lepIdx], _leptons->MuNMuonhits[lepIdx], 
                 _leptons->MuInnerTrackNHits[lepIdx], _leptons->MuInnerTrackDxy_BS[lepIdx], 
                 _leptons->vertexZ[lepIdx] - ( novtx ? 0. : _vertices->z[vtxIdx] ), 
                 _leptons->MuNPixelLayersWMeasurement[lepIdx], _leptons->MuNChambersMatchesSegment[lepIdx], code ); }
   }
