...
int
bprimeKit::GetGenMCTag( const reco::GenParticle* gen ) const
{
   genCand   = gen;
   ...
   while( genCand != NULL && genCand->numberOfMothers() == 1 ){
      ...
      if( returnTag == 0 ){
         if( abs( genCand->pdgId() )                         == 23 ){
            returnTag = 2;
         } else if( abs( genCand->pdgId() )                  == 24 ){
            returnTag = 1;
         } else if( abs( genCand->pdgId() )                  == 5  ){
            returnTag = 3;
         }// from a b quark
         else if( ( abs( genCand->pdgId() ) % 1000 ) / 100   == 5 ){
            returnTag = 3;
         }// from a B meson
         else if( ( abs( genCand->pdgId() ) % 10000 ) / 1000 == 5 ){
            returnTag = 3;
         }// from a bottom baryon
         else if( abs( genCand->pdgId() )                    == 4  ){
            returnTag = 4;
         }// from a c quark
         else if( ( abs( genCand->pdgId() ) % 1000 ) / 100   == 4 ){
            returnTag = 4;
         }// from a D meson
         else if( ( abs( genCand->pdgId() ) % 10000 ) / 1000 == 4 ){
            returnTag = 4;
         }// from a charm baryon
         else if( abs( genCand->pdgId() )                    == 15 ){
            returnTag = 7;
         }// from tau
      }
   }
