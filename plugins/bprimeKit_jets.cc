 ...
 *  Description : Fill in the jet infos
...
//   Enums and typedefs
//------------------------------------------------------------------------------ 
typedef edm::Handle<std::vector<pat::Jet>>  PATJetHandle;
typedef std::vector<PATJetHandle>           PATJetHandleList;
typedef edm::Handle<edm::ValueMap<float>>   EdmTagHandle;

//------------------------------------------------------------------------------ 
//   Helper static variables and functions
//------------------------------------------------------------------------------ 
static PATJetHandleList  JetHandle;  //PFJets
static EdmTagHandle      QGTagsHandleMLP;
static EdmTagHandle      QGTagsHandleLikelihood;
...
//------------------------------------------------------------------------------ 
void bprimeKit::fillJets( const edm::Event& iEvent , const edm::EventSetup& iSetup )
{
   ...
   for( unsigned icoll = 0; icoll < jetcollections_.size(); icoll++ ) { //loop over collections
      if( icoll >= MAX_JETCOLLECTIONS ) { break; }    // Size limitation
      if( JetHandle.size() <= icoll   ) { continue; } // Only operate if collection exists
