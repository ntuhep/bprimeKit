...
// ------------------------------------------------------------------------------
bool
bprimeKit::FillJet( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  for( unsigned icoll = 0; icoll < fJetCollections.size(); icoll++ ){// loop over collections
    ...
    const bool pfjetcoll  = ( fJetCollections[icoll] == "JetInfo" || fJetCollections[icoll] == "JetInfoPuppi" );
    const bool fatjetcoll = !pfjetcoll;
    const double pt_cut   = pfjetcoll? 15. : 100;
    const string userFloat_name   = GetUserFloatName( fJetCollections[icoll] );
    const string userFloat_prefix = GetUserFloatPrefix( fJetCollections[icoll] );

    iSetup.get<JetCorrectionsRecord>().get( fJetTypeList[icoll].c_str(), jetCorParColl );
    const JetCorrectorParameters& jetCorPar = ( *jetCorParColl )["Uncertainty"];
    JetCorrectionUncertainty jecUnc( jetCorPar );

    const string jerpt_tag  = fJetTypeList[icoll] + "_pt";
    const string jerphi_tag = fJetTypeList[icoll] + "_phi";
    const string jersf_tag  = fJetTypeList[icoll];
    ...
    for( auto it_jet = fJetList_Hs[icoll]->begin(); it_jet != fJetList_Hs[icoll]->end(); it_jet++ ){
      ...
      if( fabs( it_jet->eta() ) <= 5.0 ){
        if( fJetCorrectorList[icoll] && fJetCorUncList[icoll] ){
          fJetCorrectorList[icoll]->setJetEta( it_jet->eta() );
          fJetCorrectorList[icoll]->setJetPt( it_jet->pt() );
          fJetCorrectorList[icoll]->setJetA( it_jet->jetArea() );
          fJetCorrectorList[icoll]->setRho( *fRho_H );
          // const double unc = 1;
          const double unc = fJetCorrectorList[icoll]->getCorrection();
          fJetInfo[icoll].Unc[fJetInfo[icoll].Size] = unc;

          fJetCorUncList[icoll]->setJetEta( it_jet->eta() );
          fJetCorUncList[icoll]->setJetPt( it_jet->pt() * unc );
          fJetInfo[icoll].JesUnc[fJetInfo[icoll].Size] = fJetCorUncList[icoll]->getUncertainty( true );
        } else {
          fJetInfo[icoll].Unc[fJetInfo[icoll].Size] = jecUnc.getUncertainty( true );
          fJetInfo[icoll].JesUnc[fJetInfo[icoll].Size] = jecUnc.getUncertainty( true );
        }
      }
...
// ------------------------------------------------------------------------------
JetIterator
GetSubjetBunch( const JetIterator& main_jet, const JetHandle& subjetlist )
{
...
string
GetUserFloatName( const string& collection )
{
...
string
GetUserFloatPrefix( const string& collection )
{
  if( collection.find( "Puppi" ) != std::string::npos ){
